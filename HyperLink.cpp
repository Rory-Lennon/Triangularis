...
HCURSOR  CHyperLink::g_hLinkCursor;
...
{		    
   ...
   if (m_strURL.IsEmpty()) GetWindowText(m_strURL);
   ...
   CFont* pFont = GetFont();	
    ...
    m_ToolTip.AddTool(this, strWndText, rect, TOOLTIP_ID);
...
{
	if (BITSET(m_dwStyle,StyleGetFocusOnClick)) SetFocus();	// Set the focus and make the link active
	if (BITSET(m_dwStyle,StyleDownClick)) FollowLink();
	m_bLinkActive = TRUE;
}

void CHyperLink::OnLButtonUp(UINT /*nFlags*/, CPoint /*point*/) 
{
	if (m_bLinkActive && !BITSET(m_dwStyle,StyleDownClick)) FollowLink();
}
...
{	
	if (nChar == VK_SPACE) FollowLink();
	else CStatic::OnKeyDown(nChar, nRepCnt, nFlags);
}

void CHyperLink::SetColors() 
{
	g_crLinkColor	 = RGB(0, 0, 255);
}

void CHyperLink::SetLinkCursor(HCURSOR hCursor) 
{
	g_hLinkCursor = hCursor;
	if (g_hLinkCursor == NULL) SetDefaultCursor();
}
